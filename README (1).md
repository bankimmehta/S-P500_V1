...
## Flat (root) file layout — for GitHub web-GUI uploads
If you upload everything to the repo ROOT (no subfolders), it still works:
- `sp500_constituents.csv` is resolved from the root OR `data/` (config._resolve).
- `sp500_data.json` is resolved from the root OR `output/` (app loader).
So you can drag all files into the repo root via the GitHub GUI and the app runs.
